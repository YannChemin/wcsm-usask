...
#coeff[(coeff[,3] == 0 | is.na(coeff[,3])) & !is.na(coeff[,4]),3] = coeff[(coeff[,3] == 0 | is.na(coeff[,3])) & !is.na(coeff[,4]),4] 
coeff[is.na(coeff[,3]),3] = 0
coeff[is.na(coeff[,4]),4] = 0
...
#  val <- reactiveValues(data = read.csv(file='Radville_2009.csv'),x_coord=0,plotTitle='Norstar')
  ...
  out <- reactive({
    ...
    ode(
      ...
      parms = c(
        ...
        vernReq = max(input$vernReq,input$ltd),
        initLT50 = -3.0,
        LT50c = input$LT50c
      ),
      method = 'euler',
...
#    if(length(!is.na(val$data[,3]))==length(val$data[,1]))
#      return(val$data[,3])
#    if(dim(val$data)[2] == 3)
#      return(val$data[,3])
...
#    daylength(val$data[1,3],val$data[1,4],jdays,-6)[,3]
  ...
  output$LT50 <- renderPlot({
    ...
    out$color[!is.na(out$annotations)] = 'black'
    ...
    p = ggplot(out, aes(x = time)) +
      ...
      geom_vline(xintercept=val$x_coord,linetype=3) + 
      # geom_text(
      #   aes(
      ...
      #   size=5
      # ) +
      geom_label_repel(aes(label=annotations,y=LT50raw),
        box.padding = unit(1, "lines"),
        arrow = arrow(length = unit(0.03, "npc"), type = "closed", ends = "last"),
      ) +
      # annotate(geom='label_repel',label='Max Vernalization\nSaturation',
      #          x = out[which.max(sapply(out[, 'vernProg'], min, 1)), 'time'],
      #          y = out[which.max(sapply(out[, 'vernProg'], min, 1)), 'LT50raw'],
      ...
      #   aes(
      #     x = out[which.max(sapply(out[, 'vernProg'], min, 1)), 'time'],
      #     label = paste(ifelse(input$ltd > input$vernReq,"Low Temperature Delay: ","Max Vernalization Saturation: "), round(min(out[which.max(sapply(out[, 'vernProg'], min, 1)), 'vernProg'], 1) * 100, 2), "%", sep = ""),
      #     y = -20
      #   ),
      #   colour = "#228B22",
      #   angle = 90,
...
#        p = p + geom_vline(xintercept=val$x_coord,linetype=3)
          # annotate(
          #   geom='text',
          #   label = paste('Date:',as.Date(input$seedDate)+out[val$x_coord,'time'],
          #                 '\nDaylength:',out[val$x_coord,'daylength'], ' hours',
          #                 '\nCrown Temperature:',out[val$x_coord,'temperature'],
          #                 '\nPredicted LT50:',signif(out[val$x_coord,'LT50raw'],2)
          #   ),
          #   x = (max(out$time)+min(out$time))/2,
...
#      }
  ...
  observe({
    ...
    updateSliderInput(session,inputId='ltd',value = scoeff$V4)
